...
document.querySelector("#video-upload-button").addEventListener('click', function() {
	document.querySelector("#video-to-upload").click();
});

document.querySelector("#folder-upload-button").addEventListener('click', function() {
	document.querySelector("#folder-to-upload").click();
});

// When user chooses a MP4 file
document.querySelector("#video-to-upload").addEventListener('change', function() {
    ...
    // Hide upload button
    document.querySelector("#video-upload-button").style.display = 'none';

	// Object Url as the video source
	document.querySelector("#main-video source").setAttribute('src', URL.createObjectURL(document.querySelector("#video-to-upload").files[0]));
